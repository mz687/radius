...
def load(args):
    ...
    def _cpp_extention_load_helper(name, sources, extra_cuda_flags):
        # print(f"> Loading _cpp extension\nname:{name}\nsources:{sources}\nbuild_directory:{extra_cuda_flags}")
    ...
    if args.masked_softmax_fusion:
        ...
        # print(f"> [Rank {torch.distributed.get_rank()}] Loading masked softmax...")

        scaled_masked_softmax_cuda = _cpp_extention_load_helper(
            "scaled_masked_softmax_cuda", sources, extra_cuda_flags)

    if torch.distributed.get_rank() == 0:
        print("Finished building fused softmax")
    ...
    if torch.distributed.get_rank() == 0:
        print("Finished building Mixed precision fused layer norm.")
