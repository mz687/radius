...
'''
Implementations for all different kinds of compression algorithms are here.
[TODO] Replace PowerSGD with stable-topkA for gradient compression in data parallel.
'''
...
import math
from .utils_com import gen_threshold_from_normal_distribution
...
class GaussianCompressor():
    ...
    #        left_thres, right_thres = gen_threshold_from_normal_distribution(1-ratio, float(mean), float(std))
    #        abs_tensor = torch.abs(tensor)
