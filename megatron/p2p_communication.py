...
class PowerSVD:
    ...
    def p2p_grad_compress(self, tensor_in):
        ...
        if self.p_memory == None: # not initialized
            self.p_memory = torch.empty(p_total_size, device=self.device, dtype=tensor_in.dtype)
            self.q_memory = torch.empty(q_total_size, device=self.device, dtype=tensor_in.dtype)
